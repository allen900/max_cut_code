...
def average_performance(size, algorithm, trials=5):
    times, outputs = [], []
    best = 0
...
for pos in range(len(GRAPH_SIZES)):
    rows.append([
        ...
        qaoa_results[pos]['output'] / upper_bounds[pos]['output'],
        qaoa_results[pos]['time']
    ])

table = pd.DataFrame(rows)
# table.columns = ['Graph Size', 'Random', 'Greedy', 'SDP', 'QAOA']
table.columns = ['Graph Size', 'Alg', 'best', 'Upper_bounds', 'ratio', 'time']
